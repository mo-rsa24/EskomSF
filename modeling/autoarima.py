...
def evaluate_predictions(
    ...
    """
    Return (metrics, baseline_metrics) given actual and forecast data.
    """
...
def forecast_arima_for_single_customer(
    ...
    customer_data = customer_data[customer_data[column] == customer_id].sort_values('PodID')
    ...
    for pod_id in unique_pod_ids:
        ...
        performance_data = forecast_for_podel_id(
            podel_df, order, customer_id, pod_id, consumption_types, ufm_config, mode=mode,seasonal_order=seasonal_order,log=log
        )
...
def forecast_for_podel_id(
    ...
    model_dir = f"models/{ufm_config.forecast_method_name}/customer_{customer_id}/pod_{pod_id}"
    ...
    for consumption_type in consumption_types:
        ...
        if mode == 'validation':
            ...
            if len(final_test) > 0:
                ...
                logging.warning("⚠️ No final test data to evaluate.")
