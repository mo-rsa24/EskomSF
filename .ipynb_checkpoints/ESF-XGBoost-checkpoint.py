...
try:
    ...
    Hyper_parameter_values = Hyper_Parameters.split(',')
...
try:
    if "varJSON" in df.columns:
        ...
        # df_var.select("SelectedVarList").show(truncate=False)
    #print(all_variables)
    if not all_variables:
        raise ValueError("Schema mismtach may be found in the VarJSON column")
...
# # Plot for RMSE
# sns.barplot(data=performance_pd_df, x='PodID', y='RMSE', hue='ModelName', ax=axes[0])
...
# # Plot for R2
# sns.barplot(data=performance_pd_df, x='PodID', y='R2', hue='ModelName', ax=axes[1])
