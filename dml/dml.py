...
from db.queries import get_predictive_data, get_actual_data

# Setup logger
logging.basicConfig(level=logging.INFO)


def load_and_prepare_data(ufmd: [int] = 68,  method: str = "ARIMA", environment: str = "QA", rows: int = 5000, actual: bool = False) -> pd.DataFrame:
    """
    ...
    """
    if actual:
        csv = f"data/{environment}/ActualData{method}.csv"
    else:
...
def time_series_train_test_split(
    ...
    """
    Splits a time series into train and test sets, using a fraction of the data
    ...
    test_ratio : float, optional
        The fraction of the data to allocate to the test set. Default is 0.2 (20%).
...
def split_time_series_three_way(
    ...
    Splits the series into:
      ...
      - validation (the last `validation_ratio` portion of the remaining data)
      - sub_train (the rest)
