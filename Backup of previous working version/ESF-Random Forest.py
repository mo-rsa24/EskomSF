...
# MAGIC from pyspark.sql.types import StructType, StructField, IntegerType, StringType, FloatType,LongType
# MAGIC
...
# MAGIC
# MAGIC import miceforest as mf
# MAGIC from sklearn.impute import KNNImputer, SimpleImputer
# MAGIC from sklearn.preprocessing import LabelEncoder, OrdinalEncoder
...
# MAGIC from sklearn.metrics import accuracy_score,r2_score,make_scorer,mean_absolute_error,mean_squared_error
...
# MAGIC from pyspark.sql.functions import from_json, col, explode,collect_list,trim,regexp_replace
# MAGIC
...
# MAGIC WHERE  dbt.DatabrickID={databrick_task_id} and ufm.ForecastMethodID=13 and ExecutionStatus IN ('In Progress')
# MAGIC
# MAGIC
# MAGIC ORDER BY
...
# MAGIC  
# MAGIC
# MAGIC n_estimators = int(Hyper_Parameters.strip("()"))
...
# MAGIC     lag_columns = list(matching_key)
# MAGIC     print(f" All columns based on selectedcolumns{selected_columns}")
# MAGIC     print(f" lag columns{lag_columns}")
# MAGIC else:
...
#             if debug:
#             print(f"RMSE for Podel {podel_id}: {rmse}")
#             print(f"R² for Podel {podel_id}: {r2}")

       
#             forecast_df['rmse_results']= rmse
#             forecast_df['r2'] = r2                   

#             performance ={"ModelName" : Forecast_Method_Name,
#                         "CustomerID" : str(customer_id),
#                         "RMSE" 	     : float(rmse),
#                         "R2"	     : float(r2),
#                         "PodID"      : str(podel_id),
...
#             performance["CustomerID"] = int(performance["CustomerID"])
...
#                 StructField("DataBrickID", IntegerType(), True),  
#                 StructField("UserForecastMethodID", IntegerType(), True)
#             ])
#             display(forecast_df)

# # Use the schema when creating the DataFrame


#             forecast_combined_spark_df = spark.createDataFrame([performance], schema=schema)
...
#             # Write the DataFrame to the SQL table
#             forecast_combined_spark_df.write.jdbc(url=write_url, table=table_performance_Metrics, mode="append", properties=write_properties)
